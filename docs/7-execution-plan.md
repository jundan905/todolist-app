...
- [x] 모든 쿼리 Parameterized Query 사용 확인
...
- [x] 비밀번호 복잡도 검증 유효/무효 케이스 각 3개 이상 통과
- [x] 날짜 비교 시 timezone 이슈 없음 확인
- [x] 모든 타입이 API 명세와 일치
...
#### 완료 조건
- [x] 토큰이 `localStorage`에 저장 및 복구됨
- [x] `clearAuth()` 호출 시 토큰·사용자 정보 모두 초기화
- [x] 페이지 새로고침 후 로그인 상태 유지 확인
- [x] Zustand DevTools에서 상태 변화 추적 가능
...
#### 완료 조건
- [x] 모든 API 함수 정의 및 입출력 타입 정확히 정의
- [x] `Authorization: Bearer <token>` 헤더 자동 첨부 확인
- [x] 401 응답 시 `clearAuth()` 호출 및 로그인 페이지 리다이렉트 동작
- [x] `todoKeys` factory 패턴 구현 확인
...
#### 완료 조건
- [x] 유효한 자격증명으로 로그인 성공 및 `/todos` 이동
- [x] 실패 시 에러 메시지 표시 ("이메일 또는 비밀번호가 올바르지 않습니다")
- [x] 로딩 중 버튼 비활성화 및 스피너 표시
- [x] 비밀번호 마스킹 처리
...
#### 완료 조건
- [x] 모든 필드 입력 후 가입 성공 및 로그인 페이지 이동
- [x] 비밀번호 복잡도 미충족 시 실시간 에러 메시지 표시
- [x] 중복 이메일 시 "이미 사용 중인 이메일" 메시지 (409)
- [x] 비밀번호·비밀번호 확인 불일치 시 버튼 비활성화
...
#### 완료 조건
- [x] 모든 할일 목록 표시, 상태별 색상 구분 확인
- [x] 필터 변경 시 목록 실시간 업데이트
- [x] 정렬 변경 시 목록 재정렬
- [x] 페이지네이션 이전/다음 이동 정상 동작
- [x] 결과 없을 때 안내 메시지 표시
- [x] 로그아웃 버튼 → 로그인 페이지 이동
...
#### 완료 조건
- [x] 모든 필드 입력 후 할일 생성 성공
- [x] 제목 미입력 시 에러 메시지 표시
- [x] 종료일 < 시작일 시 에러 메시지 표시
- [x] 설명 2000자 초과 시 에러 표시
- [x] 생성 성공 후 목록 자동 업데이트
...
#### 완료 조건
- [x] 기존 값이 폼에 자동 로드
- [x] 수정 후 저장 성공 및 상세 페이지 이동
- [x] 종료일 < 시작일 시 에러 표시
- [x] isCompleted·completedAt 수정 불가 (읽기 전용)
- [x] 성공 후 상세 페이지로 자동 이동
...
#### 완료 조건
- [x] 완료 버튼 클릭 시 즉시 UI 상태 변경 (낙관적 업데이트)
- [x] 서버 응답 후 최종 상태 동기화
- [x] 기한 내 완료 → COMPLETED, 기한 초과 → LATE_COMPLETED 확인
- [x] 완료 취소 시 이전 상태 복귀
- [x] 네트워크 오류 시 롤백 및 에러 메시지 표시
...
- [x] 태블릿(768px) 레이아웃 최적화
- [x] 데스크탑(1024px+) 레이아웃 최적화
- [ ] 5가지 상태 색상 시각적 구분 확인
- [x] Tab·Enter 키보드 네비게이션 가능
...
#### 완료 조건
- [x] API 에러 발생 시 사용자 친화적 메시지 표시
- [x] 토스트 메시지 자동 표시 및 닫힘 (3~5초)
- [x] 에러 발생 후 페이지 정상 작동 (무한 로딩 없음)
...
#### 완료 조건
- [x] 비로그인 상태에서 `/todos` 접근 시 `/login` 리다이렉트
- [x] 로그인 후 `/login` 접근 시 `/todos` 리다이렉트
- [x] 존재하지 않는 경로 접근 시 404 페이지 표시
- [x] 재로그인 후 이전 URL 복귀 동작
...
#### 완료 조건
- [x] 핵심 컴포넌트 단위 테스트 작성 및 통과
- [x] SCN-01 시나리오 통합 테스트 통과
- [x] SCN-02 시나리오 통합 테스트 통과
- [x] SCN-05 상태 필터링 시나리오 통합 테스트 통과
- [x] 테스트 커버리지 70% 이상
