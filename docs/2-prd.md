...
| 언어 코드 | 언어명 | 표기 | 우선순위 |
|---|---|---|---|
| `ko` | 한국어 | 한국어 | Must-have |
| `en` | 영어 | English | Must-have |
| `ja` | 일본어 | 日本語 | Must-have |

#### 기능 요구사항

| 요구사항 | 설명 | 우선순위 |
|---|---|---|
| **언어 선택** | 헤더에서 언어 수동 선택 가능 | Must-have |
| **브라우저 연동** | 브라우저 언어 설정 감지하여 자동 적용 (기본값) | Must-have |
| **선호도 저장** | 사용자 언어 선호도 localStorage 에 저장 | Must-have |
| **전체 UI 적용** | 모든 텍스트, 버튼, 레이블, 메시지에 일관된 언어 적용 | Must-have |
| **날짜/시간 포맷** | 언어별 로컬라이즈된 날짜/시간 형식 지원 | Must-have |
| **숫자 포맷** | 언어별 로컬라이즈된 숫자 형식 지원 | Should-have |

#### 언어 동작 규칙

| 항목 | 동작 |
|---|---|
| **초기 로드** | localStorage 저장값 → 없으면 브라우저 설정 → 없으면 영어 (en) |
| **언어 변경 시** | 즉시 언어 변경 + localStorage 저장 + 페이지 리렌더링 |
| **번역 누락 시** | 영어 (en) 를 폴백 언어로 사용 |

#### 번역 리소스 관리

| 항목 | 설명 |
|---|---|
| **라이브러리** | i18next (React 용 react-i18next 사용) |
| **파일 구조** | `locales/{lang}/{namespace}.json` 형식 |
| **네임스페이스** | `common` (공통 UI), `auth` (인증), `todo` (할일), `validation` (검증) |
| **키 네이밍** | `section.element.state` 형식 (예: `todo.status.completed`) |
...
- 헤더: 로고, 네비게이션, 사용자 메뉴, **다크모드 토글 버튼**, **언어 선택 드롭다운**
